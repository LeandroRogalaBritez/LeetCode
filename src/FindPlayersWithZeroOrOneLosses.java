...
public class FindPlayersWithZeroOrOneLosses {
    ...
    private static List<List<Integer>> findWinners(int[][] matches) {
        ...
        List<Integer> notLose = winners.stream()
                .filter(winner -> losers.getOrDefault(winner, 0) == 0)
                .sorted()
                .collect(Collectors.toList());

        List<Integer> loseOneMatch = losers.entrySet().stream()
                .filter(entry -> entry.getValue() == 1)
                .map(Map.Entry::getKey)
                .sorted()
                .collect(Collectors.toList());

        return Arrays.asList(notLose, loseOneMatch);
    }
